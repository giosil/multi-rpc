...
class RpcUtil
{
  ...
  void xmlrpc_sendError(RpcServerTransport transport, int code, String message)
  {
    ...
    StringBuffer sb = new StringBuffer(246 + message.length());
  ...
  void soaprpc_sendError(RpcServerTransport transport, int code, String message, String detail)
  {
    ...
    StringBuffer sb = new StringBuffer(320 + message.length() + detail.length());
    ...
    throws Throwable
  {
    ...
    try {
      ...
      if(handler == null) {
        // [Security]
        throw new RpcRemoteException(RpcExecutor.METHOD_NOT_FOUND_ERROR_CODE, "handler " + WUtil.toHTMLText(handlerName, "") + " not found", null);
      }
      if(handler instanceof String) {
        try {
          ...
          th.printStackTrace();
          // [Security]
          throw new RpcRemoteException(RpcExecutor.METHOD_NOT_FOUND_ERROR_CODE, "handler " + WUtil.toHTMLText(handlerName, "") + " not available", null);
        }
      }
      ...
      if(method == null) {
        ...
        if(parameters == null) {
          // [Security]
          throw new RpcRemoteException(RpcExecutor.METHOD_NOT_FOUND_ERROR_CODE, "method " + WUtil.toHTMLText(methodName, "") + "(" + getStringParams(params) + ") not found", null);
        }
