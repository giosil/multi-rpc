...
class SoapRpcClient implements RpcClient, AsyncCallback
{
	...
	SoapRpcClient()
	{
		...
		try {
			this.rpcInvoker = new SoapRpcInvoker(sURL);
	...
	public
	SoapRpcClient(URL url)
	{
		this.rpcInvoker = new SoapRpcInvoker(url);
		this.rpcInvoker.getTransport().setTimeOut(iDEFAULT_TIMEOUT);
	}
	
	public
	SoapRpcClient(String sURL)
		...
		if(iTimeOut > 0) {
			...
			while(!sMethod.equals(sMethodCall)) {
				...
				if(iElapsed > iTimeOut) {
					throw new Exception(sMethod + " call timed out at " + getURL());
				}
			}
		}
		else {
